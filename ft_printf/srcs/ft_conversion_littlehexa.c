...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   ft_conversion_littlehexa.c                         :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: tdodre <tdodre@student.42lyon.fr>          +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/10/09 14:17:44 by tdodre            #+#    #+#             */
/*   Updated: 2020/10/09 14:19:31 by tdodre           ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */

#include "../include/ft_printf.h"

void		ft_write_conversion_littlehexa(long int a,
				int b, t_flags **ap, int c)
{
	if (b != 0 || (**ap).precision_exist != 1 || (**ap).precision != 0)
	{
		...
		if (a > 15)
		{
			ft_write_conversion_littlehexa(a / 16, b, ap, c);
			ft_write_conversion_littlehexa(a % 16, b, ap, c);
		}
		else if (a < 10)
			ft_write((a + 48), &ap);
...
int			ft_write_zero_littlehexa(long int a, int b, t_flags **ap)
{
...
int			ft_count_littlehexa(long int a, t_flags **ap)
{
	int		b;
...
void		ft_conversion_littlehexa(va_list args, t_flags **flags)
{
	...
	a = va_arg(args, int);
	if (a >= 0)
		b = ft_count_littlehexa(a, flags);
	else
	{
		b = 8;
		a = 4294967296 - (-a);
	}
	b = ft_write_zero_littlehexa(a, b, flags);
	if ((**flags).precision == b && a < 0)
		b++;
	if ((**flags).nb_exist == 1)
		ft_write_space((**flags).nb - b, &flags);
	c = b - ft_count_littlehexa(a, flags);
	ft_write_conversion_littlehexa(a, a, flags, c);
