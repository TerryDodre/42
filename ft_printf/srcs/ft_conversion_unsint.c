...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   ft_conversion_unsint.c                             :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: tdodre <tdodre@student.42lyon.fr>          +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/10/09 14:11:45 by tdodre            #+#    #+#             */
/*   Updated: 2020/10/09 14:14:49 by tdodre           ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */

#include "../include/ft_printf.h"

void		ft_write_conversion_unsint(long int a, int b,
	t_flags **flags, int c)
{
	if (b != 0 || (**flags).precision_exist != 1 || (**flags).precision != 0)
	{
		...
		if (a > 9)
			ft_write_conversion_unsint(a / 10, b, flags, c);
		ft_write(a % 10 + 48, &flags);
	}
}

int			ft_write_zero_unsint(long int a, int b, t_flags **ap)
{
...
int			ft_count_unsint(long int a, t_flags **ap)
{
	int		b;
...
long int	ft_check_unsigned(long int a)
{
	if (a < 0)
	{
		if (a < -2147483647)
...
void		ft_conversion_unsint(va_list args, t_flags **flags)
{
	...
	a = ft_check_unsigned(va_arg(args, long int));
	b = ft_count_unsint(a, flags);
	b = ft_write_zero_unsint(a, b, flags);
	if ((**flags).precision == b && a < 0)
		b++;
	if ((**flags).nb_exist == 1)
		ft_write_space((**flags).nb - b, &flags);
	c = b - ft_count_unsint(a, flags);
	ft_write_conversion_unsint(a, a, flags, c);
